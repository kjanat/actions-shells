...
import { writeShims } from "./shim.js";
...
function run(): void {
  ...
  if (!existsSync(cliPath)) {
    core.setFailed(
      `cli.js not found next to setup.js (${cliPath}). ` +
        "Use a release tag such as kjanat/actions-shells@v1 — master carries no built files.",
    );
    return;
  }
