import { adapters, aliases } from "./runtimes/index.js";
import type { RuntimeAdapter } from "./types.js";

const byName = new Map<string, RuntimeAdapter>(adapters.map((a) => [a.name, a]));

export function runtimeNames(): string[] {
...
export function resolveRuntime(id: string): RuntimeAdapter | undefined {
  const key = id.toLowerCase();
  return byName.get(key) ?? byName.get(aliases[key] ?? "");
}

export function allRuntimes(): readonly RuntimeAdapter[] {
  return adapters;
}
