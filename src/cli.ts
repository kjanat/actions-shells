import { describeInput, doctor, formatReport } from "./doctor.js";
import { aliasesFor, allRuntimes, envKey, resolveRuntime } from "./registry.js";
import { RuntimeNotFoundError, runScript } from "./run.js";
...
function listText(): string {
  ...
  return rows
    .map(
      (r) =>
        `${(r[0] ?? "").padEnd(widths[0] ?? 0)}  ${(r[1] ?? "").padEnd(widths[1] ?? 0)}  ${(r[2] ?? "").padEnd(widths[2] ?? 0)}${r[3] ? `  aliases: ${r[3]}` : ""}`,
    )
    .join("\n");
}

function unknownRuntime(id: string): number {
...
async function main(argv: string[]): Promise<number> {
  ...
  if (first === "describe") {
    ...
    const lines = [
      ...
      `command:            ${adapter.command}${adapter.fallbackCommands?.length ? ` (fallbacks: ${adapter.fallbackCommands.join(", ")})` : ""}`,
      `input:              ${input}`,
  ...
  if (first === "doctor") {
    ...
    if (id) {
      ...
      if (report.status !== "ready" && process.env.GITHUB_ACTIONS === "true") {
        out(
          `::warning title=actions-shell::runtime ${adapter.name} is ${report.status} (${report.command})`,
        );
      }
