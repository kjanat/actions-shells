...
- Runtimes: node, deno, bun, rust-script, cargo-script, go, kotlin, swift, csharp, fsharp, python, ruby, perl, php, lua, julia, r.
- Aliases: `rust` (→ rust-script), `js`, `ts`, `cs`, `fs`, `kt`, `rb`, `pl`, `jl`, `py`.
- `list`, `describe <runtime>`, `doctor [runtime]`, `--version`.
- Environment overrides `ACTIONS_SHELL_<RUNTIME>_COMMAND` / `_ARGS`, plus `ACTIONS_SHELL_DEBUG` and `ACTIONS_SHELL_KEEP`.
- Setup action (`runs.using: node24`) that puts `actions-shell` on `PATH` for Linux, macOS and Windows runners, with optional `doctor` / `runtimes` inputs.
