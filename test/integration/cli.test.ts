...
import { resolveRuntime } from '#registry';
import { resolveCommand } from '#run';
...
function available(runtime: string): boolean {
	...
	if (a.name === 'cargo-script') {
		const probe = spawnSync('cargo', ['+nightly', '--version'], { encoding: 'utf8', windowsHide: true });
...
describe('actions-shell <runtime> <file>', () => {
	for (const [i, c] of cases.entries()) {
		...
		it.skipIf(!ok)(`${c.runtime}: prints and exits ${c.exit}`, () => {
			...
			const leftovers = ['', '.cs', '.fsx', '.go', '.go.bin', '.go.exe', '.main.kts', '.rs', '.swift', '.ts']
				.map((e) => f + e)
