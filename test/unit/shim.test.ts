...
import { shimSources, writeShims } from "../../src/shim.js";

describe("shims", () => {
  it("quote paths with spaces and apostrophes", () => {
    const { posix, cmd } = shimSources({
      binDir: "/b",
      nodePath: "/opt/host ed/node",
      cliPath: "/tmp/it's/cli.js",
    });
  ...
  it("write both shim files, executable on posix", () => {
    ...
    try {
      const written = writeShims({
        binDir: path.join(dir, "bin"),
        nodePath: process.execPath,
        cliPath: "/c.js",
      });
      expect(written.map((w) => path.basename(w)).sort()).toEqual([
        "actions-shell",
        "actions-shell.cmd",
      ]);
